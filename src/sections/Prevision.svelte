<script lang="ts">
  ...
  type GastoFijo = {
    ...
    medioPago?: string; // Medio de pago o destinatario
  };
  ...
  onMount(() => {
    ...
    if (storedCuotas) {
      ...
      cuotas = [
        ...
        {
          id: crypto.randomUUID(),
          concepto: "Patente Focus SE Plus",
          montoTotal: 700000,
          montoCuota: 70000,
          cuotasTotal: 10,
          cuotasPagadas: 0,
  ...
  function agregarCuota() {
    if (
      !nuevaCuota.concepto ||
      nuevaCuota.montoTotal <= 0 ||
      nuevaCuota.cuotasTotal <= 0
    )
      return;
    ...
    const cuota: Cuota = {
      ...
      medioPago: nuevaCuota.medioPago,
    };
  ...
  function cuotaActivaEnMes(cuota: Cuota, mesIndex: number): boolean {
    ...
    return (
      ...
      mesesTranscurridos < cuota.cuotasTotal - cuota.cuotasPagadas
    );
  }
  ...
  function calcularDiferenciaMes(mesIndex: number): number {
    return (
      calcularTotalIngresosMes(mesIndex) - calcularTotalGastosMes(mesIndex)
    );
  }
  ...
  function agruparGastosPorCategoria() {
    ...
    gastosFijos.forEach((gasto) => {
      ...
      console.log(
        `Procesando gasto: ${gasto.concepto} - Categoría: ${categoria}`
      );
    ...
    cuotas.forEach((cuota) => {
      ...
      console.log(
        `Procesando cuota: ${cuota.concepto} - Categoría: ${categoria}`
      );
  ...
  $: totalGastosFijos = gastosFijos.reduce(
    (total, gasto) => total + gasto.monto,
    0
  );

  // Ordenar gastos fijos por vencimiento
  $: gastosFijosOrdenados = [...gastosFijos].sort(
    (a, b) => a.vencimiento - b.vencimiento
  );

  // Calcular totales por mes para la tabla de previsión
  $: totalesGastosPorMes = meses.map((_, index) =>
    calcularTotalGastosMes(index)
  );
  $: totalesIngresosPorMes = meses.map((_, index) =>
    calcularTotalIngresosMes(index)
  );
  $: diferenciasPorMes = meses.map((_, index) => calcularDiferenciaMes(index));

  // Calcular equivalente en USD
  $: totalesGastosUSD = totalesGastosPorMes.map((total) =>
    Math.round(total / valorDolar)
  );
  $: totalesIngresosUSD = totalesIngresosPorMes.map((total) =>
    Math.round(total / valorDolar)
  );
  $: diferenciasUSD = diferenciasPorMes.map((total) =>
    Math.round(total / valorDolar)
  );
...
<div>
  ...
  {#if activeTab === "prevision"}
    <div>
      <!-- Tabla 1: Sistema, Ingresos, Gastos, Conversión y Balance -->
      <h3 class="text-xl font-medium mb-4">
        Previsión Mensual de Ingresos y Gastos
      </h3>
      ...
      <div class="overflow-x-auto mb-8">
        <table class="min-w-full divide-y divide-gray-200 border">
          <thead class="bg-gray-100">
            <tr>
              <th
                class="px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-r"
                >Concepto</th
              >
              {#each meses as mes}
                <th
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr class="bg-gray-50">
              ...
              {#each meses as _, i}
                <td
                  class="px-2 py-2 whitespace-nowrap text-sm text-center text-gray-500 border-r"
                  >{valorDolar}</td
                >
              {/each}
            </tr>
            ...
            <!-- Sueldo -->
            {#each ingresos.filter((ing) => ing.concepto === "Sueldo" || ing.concepto === "Post Financiera") as ingreso}
            ...
            <tr class="bg-green-100">
              ...
              {#each totalesIngresosPorMes as total}
                <td
                  class="px-2 py-2 whitespace-nowrap text-sm text-center font-bold text-gray-900 border-r"
                  >{formatMonto(total)}</td
                >
              {/each}
            </tr>
            ...
            <tr class="bg-red-100">
              <td
                class="px-2 py-3 whitespace-nowrap text-sm font-medium text-gray-900 border-r"
                >Total Gastos ARS</td
              >
              {#each totalesGastosPorMes as total}
                <td
                  class="px-2 py-3 whitespace-nowrap text-sm text-center text-gray-500 border-r"
                  >{formatMonto(total)}</td
                >
              {/each}
            </tr>
            ...
            <tr class="bg-blue-100">
              ...
              {#each totalesGastosPorMes as total, i}
                <td
                  class="px-2 py-2 whitespace-nowrap text-sm text-center text-gray-500 border-r"
                  >{formatMonto(totalesGastosUSD[i] * valorDolar)}</td
                >
              {/each}
            </tr>
      ...
      <!-- Tabla 2: Detalle de Ingresos y Gastos -->
      <h3 class="text-xl font-medium mb-4">Detalle de Ingresos y Gastos</h3>
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200 border">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <!-- Ingresos (excluyendo Sueldo y Post Financiera) -->
            {#each ingresos.filter((ing) => ing.concepto !== "Sueldo" && ing.concepto !== "Post Financiera") as ingreso}
              <tr class="bg-green-50">
                <td
                  class="px-2 py-2 whitespace-nowrap text-sm font-medium text-gray-900 border-r"
            ...
            <!-- Gastos Fijos -->
            {#each gastosFijos as gasto}
              <tr class="bg-red-50">
                <td
                  class="px-2 py-2 whitespace-nowrap text-sm font-medium text-gray-900 border-r"
                  >{gasto.categoria || "Sin categoría"}</td
                >
                <td
                  class="px-2 py-2 whitespace-nowrap text-sm font-medium text-gray-900 border-r"
                  >{gasto.categoria === "Transferencia" ? "Transferencia" : "Tarjeta"}</td
                >
                <td
                  class="px-2 py-2 whitespace-nowrap text-sm font-medium text-gray-900 border-r"
                  >{gasto.concepto}</td
            ...
            {#each cuotas as cuota}
              <tr class="bg-orange-50">
                <td
                  class="px-2 py-2 whitespace-nowrap text-sm font-medium text-gray-900 border-r"
                  >{cuota.categoria || "Cuotas"}</td
                >
                <td
                  class="px-2 py-2 whitespace-nowrap text-sm font-medium text-gray-900 border-r"
                  >{cuota.categoria === "Auto" ? "Transferencia" : "Tarjeta de Crédito"}</td
                >
                <td
                  class="px-2 py-2 whitespace-nowrap text-sm font-medium text-gray-900 border-r"
                  >{cuota.concepto}</td
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
          ...
          <div>
            <label
              for="duracion"
              class="block text-sm font-medium text-gray-700 mb-1"
              >Duración (meses, 0 = permanente)</label
            >
            <input
              type="number"
              id="duracion"
        ...
        <div class="grid grid-cols-1 md:grid-cols-1 gap-4 mt-4">
          <div>
            <label
              for="medioPago"
              class="block text-sm font-medium text-gray-700 mb-1"
              >Medio de Pago / Destinatario</label
            >
            <input
              type="text"
              id="medioPago"
      ...
      <div>
        <div class="flex justify-between items-center mb-4">
          ...
          <p class="text-sm font-medium">
            Total: {formatMonto(totalGastosFijos)}
          </p>
        </div>
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                {#each gastosFijosOrdenados as gasto}
                  <tr class={gasto.pagado ? "bg-gray-50" : ""}>
                    ...
                    <td
                      class="px-4 py-4 whitespace-nowrap text-sm text-gray-500"
                      >{formatMonto(gasto.monto)}</td
                    >
                    <td
                      class="px-4 py-4 whitespace-nowrap text-sm text-gray-500"
                      >Día {gasto.vencimiento}</td
                    >
                    <td
                      class="px-4 py-4 whitespace-nowrap text-sm text-gray-500"
                    >
                    ...
                    >
                      <label class="inline-flex items-center">
                        ...
                        />
                        <span class="ml-2"
                          >{gasto.pagado ? "Pagado" : "Pendiente"}</span
                        >
                      </label>
                    </td>
                    <td
        ...
        <div class="grid grid-cols-1 md:grid-cols-1 gap-4 mt-4">
          <div>
            ...
            <input
              type="text"
              id="medioPagoCuota"
              bind:value={nuevaCuota.medioPago}
              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500"
            />
          </div>
        </div>
              ...
              {#each cuotas as cuota}
                <tr>
                  ...
                  <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-500"
                    >{cuota.categoria || "Sin categoría"}</td
                  >
                  <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-500"
                    >{formatMonto(cuota.montoTotal)}</td
                  >
                  <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-500"
                    >{formatMonto(cuota.montoCuota)}</td
                  >
                  <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-500"
                    >{cuota.cuotasPagadas} / {cuota.cantidadCuotas}</td
                  >
                  <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-500"
                    >Día {cuota.vencimiento}</td
                  >
                  <td
                    class="px-4 py-4 whitespace-nowrap text-right text-sm font-medium"
                  >
    ...
    <div>
      ...
      <div class="bg-white p-4 rounded-md shadow mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            <label
              for="vencimientoIngreso"
              class="block text-sm font-medium text-gray-700 mb-1"
              >Día de ingreso</label
            >
            <input
              type="number"
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              {#each ingresos as ingreso}
                <tr>
                  ...
                  >
                  <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-500"
                    >{formatMonto(ingreso.monto)}</td
                  >
                  <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-500"
                    >${ingreso.montoUSD ||
                      (ingreso.monto / valorDolar).toFixed(2)}</td
                  >
                  <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-500"
                    >Día {ingreso.vencimiento}</td
                  >
                  <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-500">
                    {#if ingreso.duracion && ingreso.duracion > 0}
                      {ingreso.duracion}
                      {ingreso.duracion === 1 ? "mes" : "meses"}
                    {:else}
                      Permanente
                    {/if}
                  </td>
